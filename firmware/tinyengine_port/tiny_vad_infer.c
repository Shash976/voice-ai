...
/* ── Acceleration hooks (NULL = use software path) ──────────────────────────── */

tinyvad_conv1d_fn tinyvad_conv1d_hook = NULL;
tinyvad_dense_fn  tinyvad_dense_hook  = NULL;
...
static void conv1d(
    ...
    int relu)
{
    if (tinyvad_conv1d_hook) {
        tinyvad_conv1d_hook(inp, w, b, out,
...
static void dense(
    ...
    int relu)
{
    if (tinyvad_dense_hook) {
        tinyvad_dense_hook(inp, w, b, out,
                           in, out_dim, in_zp, out_zp,
                           q_mult, rshift, relu);
        return;
    }
