        ...
        convert_to_tflite.py
          └─> tiny_vad_int8.tflite    (quantized model — the source of truth)
                ...
                physical/orfs/make/  → OpenROAD-flow-scripts → GDS
                      (LANES=4 ACC_W=24: ~19,738 µm², ~269 MHz Fmax, 230 FFs)
```
...
| Stage 4 — accelerator, 8 MAC lanes | ~61.4 K | ~0.61 ms | **~182×** | 64/64 |
| Stage 4 — accelerator, 16 MAC lanes | ~46.7 K | ~0.47 ms | **~240×** | 64/64 |

(Measured with the RTL-matched cycle model; the per-lane table lives in
`optimizer/constants.py`, regenerated by `measure_real.py`.)
...
The ~182× behavioral speedup plus the physical numbers together answer the key
